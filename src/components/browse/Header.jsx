...
const Header = () => {
  return (
    <header className='bg-[#141414] fixed top-0 w-full flex items-center justify-between px-4 py-4 lg:px-10 lg:py-6 z-50 '>
      <div className='flex items-center space-x-2 md:space-x-10 '>
        ...
        <div className='flex'>
          ...
          <Link to="/browse" className="link-style">Movies</Link>
          <Link to="/browse" className="link-style">Latest</Link>
          <Link to="/browse" className="link-style">MyList</Link>
        </div>
      </div>
      <div className='flex items-center gap-4 text-sm'>
        < AiFillBell className="h-6 w-6 cursor-pointer" />
        < AiOutlineSearch className="h-6 w-6 cursor-pointer" />
        <img src="./images/logo/profile.png" className='h-[40px] w-[40px] cursor-pointer rounded' />
      </div>
    </header>
  )
}
